...
function getConfig() {
  return {
    ...
    oaHeaders: getOaHeaders(),
    erpHeaders: getErpHeaders()
  };
}

function getDifferenceTypePriority() {
  return [
    "OA有申请，ERP无出库",
...
function buildSummaryRows(detailRows) {
  ...
  var grouped = {};
  var order = [];
  var differenceTypePriority = getDifferenceTypePriority();
  var index;
  var row;
  var key;
  var summary;
  ...
  for (index = 0; index < order.length; index += 1) {
    ...
    for (
      typeIndex = 0;
      typeIndex < differenceTypePriority.length;
      typeIndex += 1
    ) {
      if (summary._differenceTypes[differenceTypePriority[typeIndex]]) {
        summaryTypes.push(differenceTypePriority[typeIndex]);
      }
    }
...
function writeObjectValue(target, value) {
  ...
  // WPS 某些版本把 Value 暴露为只读；优先写 Value2，减少运行时兼容问题。
  try {
    target.Value2 = value;
    return;
  } catch (error) {
...
function setupQueryPanel() {
  var sheet = ensureSheet(getConfig().sheets.panel);
